...
Ce document est un aide-mémoire pour réussir l'importation de ces données. Suivant les différentes étapes et tout devrait bien se passer.
L'objectif est ici d'importer le jeu de données [samares](datasets/samares_small.xlsx) disponible sur la page des [données](data.html)
...
![Créer un nouveau projet](img/h_creer_nouveau_projet.png){ width=50% }

Depuis un nouveau répertoire

![Un nouveau répertoire pour le projet](img/h_nouveau_repertoire.png){ width=50% }

pour faire un projet standard 

![Un projet standard](img/h_nouveau_projet.png){ width=50% }

qui s'appelera Td_Stat_1 et sera placé dans le répertoire désigné

![Un projet standard](img/h_nom_projet.png){ width=50% }


## Etape 2 : Aller chercher le fichier
...
Les lignes qui commencent par `#` ne sont pas interpréter par R, elles permettent de faire des commentaires.
...
On peut aussi demandé un résumé du jeu de données qui va nous aider à vérifier que l'importation s'est bien déroulée.

```{r}
summary(dta)
```

Normalement Les variables quantitatives sont résumés quantitativement par le minimuma, le premier quartile, la médiane, etc .... Tandis que les variables qualitatives sont interprétées comme des facteurs et sont résumées par le nombre d'occurence dans chaque niveau du facteur.

Si toutes les vérifications sont correctes, votre jeu de données est prêt à être analysé.

## Etape 7 : ce qui peut mal se passer 
...
Si le séparateur de colonnes  alors il faut modifier l'argument `sep`
...
### Problème avec les varaibles numériques

Il est possible avec une version française d'Excel, que le symbole qui code pour la décimale ne soit pas le `.` qui est la convention interbnationale mais le `,` qui est l'habitude Française. Si on ne l'indique pas à R, il ne va pas comprendre que la variable concernée est quantitative. Ceci sera visible dans le résumé de la variable. Si tel est le cas, il suffit de rajouter l'argument `dec = ',' ` dans la fonction `read.table` (à n'importe quel endroit ). 
